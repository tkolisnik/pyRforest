...
#' Note: Plot is based on ggplot2, and after saving the plot object 'p' you can use ggplot2 commands to modify it further, e.g. 'p2 <- p + ggplot2::theme_gray()' to change background colour.
#'
...
#' @param showlegend logical TRUE or FALSE, whether or not to include the legend
#' @import ggplot2
#' @import dplyr
#' @examplesIf FALSE
...
#' @export
generate_fi_rank_plot<-function(permutedvalues,quantiledata,xlimitmin=1,xlimitmax=500,ylimitmin=-10,ylimitmax=0,xtickbreaks=10,labelverticaladjust=1.05,labelhorizontaladjust=1.05,indvPermScoresOn=TRUE, logOn=TRUE, focusedView=FALSE, showlegend=TRUE){
  # Initialize Data frame for manual legend
  legend_data <- data.frame(Legend = c('Observed Importances', 'Permuted Importances (Mean)',
                                      'Confidence Interval', 'Permuted Importances (Individual)',
                                      'Significance Threshold'))
  # Specify order of legend and convert to factor #fun ggplot nuances...
  legend_data$Legend <- factor(legend_data$Legend, levels = c('Observed Importances',
                                                              'Permuted Importances (Mean)',
                                                              'Confidence Interval',
                                                              'Permuted Importances (Individual)',
                                                              'Significance Threshold'))
  ...
  if(focusedView==FALSE){ # FULL View
    ...
    message("Warnings regarding rows containing missing values (`geom_line()`) are related to values plotted outside axis limits and can be ignored once you are happy with the way your plot looks.")
      ...
      theme_linedraw() +
      NULL
      p<-p+ geom_point(data = legend_data, aes(x = 0, y = 0, color = Legend), show.legend = showlegend) +
        scale_color_manual(values = c('Observed Importances' = 'gold3', 'Permuted Importances (Mean)' = 'black', 'Confidence Interval' = 'grey20', 'Permuted Importances (Individual)' = 'lightblue', 'Significance Threshold' = 'red')) +
...
generate_shap_plots <- function(mean_shap_values, long_shap_data, title_global = "SHAP Global Feature Importance",
  ...
  bar_plot <- ggplot2::ggplot(mean_shap_values, aes(x = feature, y = abs_mean_shap, fill = mean_shap > 0)) +
    geom_col() +
    coord_flip() +
  ...
  beeswarm_plot <- ggplot2::ggplot(long_shap_data, aes(x = feature, y = shap_value, color = shap_value)) +
    ggbeeswarm::geom_quasirandom(size = 1, alpha = 0.5) +
    coord_flip() +
    scale_y_continuous() +
    #scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.1, 0.05)) +
  ...
  combined_plot <- cowplot::plot_grid(bar_plot, beeswarm_plot, align = 'h', ncol = 2, rel_widths = c(0.5, 1))
