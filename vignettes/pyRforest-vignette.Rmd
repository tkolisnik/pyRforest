...
This package has been developed on Apple Mac (M1 arm64). It has been tested on arm64 Mac, Windows 10 (Intel x64), and Linux (Ubuntu) machines. 
Parallelization of model building does not work on AMD-based Windows machines due to incompatibilities with the underlying packages. In addition this package has been verified to run on [posit Cloud](https://posit.cloud/).

This package leverages [scikit-learn's](https://scikit-learn.org/stable/) random forest model using the R package reticulate to allow us to execute [Python](https://www.python.org/) code within [R](https://www.r-project.org/about.html). Therefore, in order to use this package you must first install python 3.9.18 and [reticulate](https://rstudio.github.io/reticulate/) version 1.34.0 recommended, and optionally [conda](https://conda.io/projects/conda/en/latest/user-guide/install/index.html), and setup either a reticulate virtualenv (linux) or a conda environment (mac or windows) with python 3.9.18, scikit-learn 3.1.2, numpy 1.26.0, shap 0.43.0, scikit-optimize 0.10.2, memory_profiler 0.61.0, and their dependencies.

# Mac Installation with Conda
```{r, eval=FALSE, tidy=TRUE}
# Verified on M1 chip Arm64 Macs

# Download and install miniconda for Mac from: https://docs.anaconda.com/miniconda/ 
# Anaconda also works - the two are interchangeable. 
# Open a terminal window

# Locate your conda install by typing the following into terminal:
which conda

# Run the following command in terminal to create your conda environment
conda create -n pyRforest-conda -c conda-forge python=3.9.18 scikit-learn=1.3.2 numpy=1.26.0 
shap=0.43.0 scikit-optimize=0.10.2 memory_profiler=0.61.0 

# Press 'y' during install to accept installation of dependencies. 
...
# Replace the path for the conda variable with the path 
# found from typing 'which conda' into terminal
reticulate::use_condaenv("pyRforest-conda", 
                         conda = '/opt/homebrew/Caskroom/miniconda/base/bin/conda', required = TRUE)

# You can check if environment setup has worked by running:
reticulate::py_config()

# If environment installation has worked,
# the previous command will result in an output message similar to:
...
```{r, eval=FALSE, tidy=TRUE}
# Verified on Windows 10 x64 Intel architecture - parallelization unlikely to work on AMD architecture 

# Download and install miniconda for Windows from: https://docs.anaconda.com/miniconda/ 
# Anaconda also works - the two are interchangeable.

# Open an Anaconda Prompt window by typing Anaconda Prompt into the search bar

# Locate your conda install by typing the following into the Anaconda Prompt:
conda info --base

# Run the following command in Anaconda Prompt to create your conda environment
conda create -n pyRforest-conda -c conda-forge python=3.9.18 scikit-learn=1.3.2 numpy=1.26.0 
shap=0.43.0 scikit-optimize=0.10.2 memory_profiler=0.61.0 
...
# Replace the path for the conda variable with the path found 
# from typing conda info --base into Anaconda Prompt
reticulate::use_condaenv("pyRforest-conda", 
                         conda = 'C:/Users/USER/anaconda3/condabin/conda', required = TRUE)
...
reticulate::virtualenv_create(envname = "pyRforest-venv", python_version = "3.9.18", 
                              packages = c("scikit-learn==1.3.2", "numpy==1.26.0", 
                                           "shap==0.43.0","scikit-optimize==0.10.2",
...
library(reticulate)
# Install python 3.9.18 if not already installed by downloading it from: https://www.python.org/downloads/macos/ 
# Ensure it is installed to the folder: /usr/local/bin/python3. 
...
<p>To make full use of the machine-learning tools provided in this package, we recommend structuring your expression and target data in a specific format. Below, we outline how to organize your data to mirror our demo dataset, ensuring compatibility with the package's functionality.</p>

<p>Your data should be stored as a list containing four tibbles. Each tibble behaves like a data frame but provides additional flexibility, especially when dealing with large datasets. Here is a breakdown of the structure and purpose of each tibble:</p>

<h4>**Training, Validation, and Testing Tibbles**</h4>
These three tibbles represent the splits of your data used for training, validating, and testing your machine learning models. Each tibble must contain the following columns:
<ul>

<li><strong>identifier:</strong> A unique identifier for each row (e.g., "2041-04-12-1"), representing a sample or patient. This is column is for tracking and referencing data points.</li>

<li><strong>target:</strong> A variable indicating the outcome/target to be predicted. May be binary, categorical (including more than 2 classes), but must not be continuous. </li>

<li><strong>training_data | testing_data | validation_data:</strong> The remaining columns, such as ENSG00000253497 and ENSG00000238493, contain gene expression data, here represented as RNA-seq counts. Each column corresponds to a specific gene.</li>

<h4>**Target Categories Tibble**</h4>
The target categories tibble provides the categorical classification for each sample in your dataset. This is particularly useful when working with multiclass problems. It contains the following columns:

<li><strong>identifier:</strong> A unique identifier for each row (e.g., "2041-04-12-1"), representing a sample or patient. This is column is for tracking and referencing data points, matches the tibbles above for linking, but contains the full data.</li>

<li><strong>target_categorical:</strong> Categorical variable representing the class labels, e.g. ("Left", "Right") </li>

<li><strong>target:</strong> Numeric variable representing the class labels, e.g. ("0", "1") </li>
</ul>
...
<li><strong>Optional Testing Dataset:</strong> Used to provide an unbiased evaluation of the final model fit. It's only used after the model training and validation phases are complete.</li>
...
<p>[It's a common practice](https://encord.com/blog/train-val-test-split/) to use around 60-80% of the data for training, and the remaining 20-40% split between validation and testing. However, these ratios can vary based on the size and specifics of your dataset.</p>
...
tuning_results <- pyRforest::tune_and_train_rf_model_grid(
                                  ...
                                  scoring = 'roc_auc',
                                  seed = 4,
                                  param_grid = NULL, # If null uses a comprehensive default grid
                                  n_jobs = 1,
                                  n_cores = -2
                                  )
...
# Print top features from model prior to rank-based feature reduction
...
# If a models classes are: ['Left' 'Right'] their order represents the index (starting at 0) 
# which should be passed to class_index
